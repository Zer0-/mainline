...
word16_bytestring_bijection :: ByteString -> Bool
word16_bytestring_bijection b = bytes == pack (octets w)
    where
        w :: Word16
        w = fromByteString b
        bytes = pack $ take 2 (extendListWith (unpack b) 0)

word32_bytestring_bijection :: ByteString -> Bool
word32_bytestring_bijection b = bytes == pack (octets w)
    where
        w :: Word32
        w = fromByteString b
        bytes = pack $ take 4 (extendListWith (unpack b) 0)

word160_bytestring_bijection :: ByteString -> Bool
word160_bytestring_bijection b = bytes == pack (octets w)
    where
        w :: Word160
        w = fromByteString b
        bytes = pack $ take 20 (extendListWith (unpack b) 0)
